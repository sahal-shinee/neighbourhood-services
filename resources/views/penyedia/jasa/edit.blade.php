...
@php
    $existingPaket = old('paket')
        ? collect(old('paket'))->values()->map(fn($p, $i) => array_merge($p, ['id' => $i]))->toArray()
        : $jasa->paket->values()->map(fn($p, $i) => ['id' => $i, 'nama_paket' => $p->nama_paket, 'harga' => (float)$p->harga, 'deskripsi' => $p->deskripsi ?? ''])->toArray();
    
    if (empty($existingPaket)) {
        $existingPaket = [['id' => 0, 'nama_paket' => '', 'harga' => '', 'deskripsi' => '']];
    }
    
    $existingTipe = old('tipe_tarif', $jasa->tipe_tarif ?? 'per_jam');
@endphp

<div class="max-w-3xl mx-auto" x-data="jasaForm('{{ $existingTipe }}')">
    <div class="bg-white rounded-[2.5rem] border border-gray-100 shadow-[0_4px_24px_rgba(0,0,0,0.04)] overflow-hidden">
        ...
        <div class="h-28 bg-gradient-to-r from-indigo-600 to-indigo-700 relative flex items-end px-8 pb-6">
            ...
            <div class="relative text-white">
                <h3 class="text-lg font-black tracking-tight">Edit Layanan</h3>
                <p class="text-xs text-indigo-200 font-medium mt-0.5 truncate max-w-xs">{{ $jasa->nama_jasa }}</p>
            </div>
        </div>

        <form action="{{ route('penyedia.jasa.update', $jasa->id_jasa) }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-7">
            @csrf
            @method('PUT')

            {{-- Kategori --}}
            <div>
                <label class="block text-xs font-black text-gray-700 uppercase tracking-wider mb-2">Kategori Layanan <span class="text-rose-500">*</span></label>
                <select name="kategori_jasa" required
                    ...
                    @foreach($kategori as $k)
                        <option value="{{ $k->nama_kategori }}" {{ old('kategori_jasa', $jasa->kategori_jasa) == $k->nama_kategori ? 'selected' : '' }}>
            ...
            {{-- Nama Jasa --}}
            <div>
                <label class="block text-xs font-black text-gray-700 uppercase tracking-wider mb-2">Nama Layanan <span class="text-rose-500">*</span></label>
                <input type="text" name="nama_jasa" value="{{ old('nama_jasa', $jasa->nama_jasa) }}" required
                    class="block w-full rounded-2xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm font-medium text-gray-900 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-400/10 focus:bg-white outline-none transition-all">
            ...
            {{-- Deskripsi --}}
            <div>
                <label class="block text-xs font-black text-gray-700 uppercase tracking-wider mb-2">Deskripsi Layanan <span class="text-rose-500">*</span></label>
                <textarea name="deskripsi_jasa" rows="4" required
                    class="block w-full rounded-2xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm font-medium text-gray-900 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-400/10 focus:bg-white outline-none transition-all resize-none">{{ old('deskripsi_jasa', $jasa->deskripsi_jasa) }}</textarea>
                @error('deskripsi_jasa') <p class="mt-1.5 text-xs text-rose-500 font-semibold">{{ $message }}</p> @enderror
            </div>

            {{-- Tipe Tarif --}}
            <div>
                <label class="block text-xs font-black text-gray-700 uppercase tracking-wider mb-3">Model Tarif <span class="text-rose-500">*</span></label>
                    ...
                    @foreach([
                        ...
                        <label class="cursor-pointer">
                            ...
                            <div :class="tipeTarif === '{{ $val }}'
                                ...
                                <p class="text-xs font-black" :class="tipeTarif === '{{ $val }}' ? 'text-indigo-700' : 'text-gray-700'">{{ $opt['label'] }}</p>
                                <p class="text-[10px] font-medium mt-0.5" :class="tipeTarif === '{{ $val }}' ? 'text-indigo-400' : 'text-gray-400'">{{ $opt['desc'] }}</p>
                            </div>
                        </label>
                    @endforeach
                </div>
            ...
            <div x-show="tipeTarif !== 'paket'" x-transition:enter="transition ease-out duration-200"
                ...
                <div class="relative">
                    ...
                    <input type="number" name="tarif_per_jam"
                        value="{{ old('tarif_per_jam', $jasa->tarif_per_jam) }}"
            ...
            <div x-show="tipeTarif === 'paket'" x-transition:enter="transition ease-out duration-200"
                ...
                <div class="flex items-center justify-between mb-3">
                    <div>
                        <label class="block text-xs font-black text-gray-700 uppercase tracking-wider">Daftar Paket / Tier <span class="text-rose-500">*</span></label>
                        <p class="text-[11px] text-gray-400 font-medium mt-0.5">Buat 1–5 paket. Perubahan akan menimpa semua paket sebelumnya.</p>
                    </div>
                    <button type="button" @click="addPaket()" x-show="pakets.length < 5"
                        ...
                        <div class="bg-gray-50 rounded-2xl border border-gray-200 p-5 relative group">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" :name="`paket[${index}][nama_paket]`" x-model="paket.nama_paket"
                                ...
                                <div>
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="number" :name="`paket[${index}][harga]`" x-model="paket.harga"
                                            min="1000" step="1000"
                                ...
                                <div class="col-span-2">
                                    <label class="block text-[10px] font-black text-gray-500 uppercase tracking-wider mb-1.5">Deskripsi / Benefit <span class="font-normal text-gray-400 normal-case">(opsional)</span></label>
                                    <textarea :name="`paket[${index}][deskripsi]`" x-model="paket.deskripsi" rows="3"
            ...
            {{-- Foto Jasa --}}
            <div>
                <label class="block text-xs font-black text-gray-700 uppercase tracking-wider mb-2">Foto Thumbnail Layanan</label>
                @if($jasa->foto_jasa)
                    <div class="mb-3 flex items-center gap-3 p-3 bg-gray-50 rounded-xl border border-gray-100">
                        <img src="{{ $jasa->foto_jasa_url }}" alt="" class="w-14 h-14 rounded-xl object-cover">
                        <div>
                            <p class="text-xs font-bold text-gray-700">Foto saat ini</p>
                            <p class="text-[11px] text-gray-400 mt-0.5">Upload foto baru untuk mengganti.</p>
                        </div>
                    </div>
                @endif
                <input type="file" id="foto_jasa" name="foto_jasa" accept="image/jpeg,image/png,image/jpg,image/webp"
                    class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100 transition-all">
                <p class="text-[11px] text-gray-400 font-medium mt-1.5">Biarkan kosong jika tidak ingin mengubah. Maks 2MB.</p>
            ...
            <div class="flex items-center gap-3 p-4 bg-gray-50 rounded-2xl border border-gray-100">
                <input id="is_aktif" name="is_aktif" type="checkbox" value="1"
                    {{ old('is_aktif', $jasa->is_aktif) ? 'checked' : '' }}
                    class="w-4 h-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500">
                <div>
                    <label for="is_aktif" class="text-sm font-bold text-gray-900 cursor-pointer">Layanan Aktif</label>
                    <p class="text-[11px] text-gray-400 font-medium">Layanan aktif akan tampil di pencarian pelanggan.</p>
                </div>
            </div>
...
function jasaForm(initialTipe) {
    return {
        ...
        pakets: @json($existingPaket),
        _nextId: {{ count($existingPaket) }},
        addPaket() {
